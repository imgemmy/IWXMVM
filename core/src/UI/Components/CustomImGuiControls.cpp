...
namespace ImGui
{
    ...
    bool TimescaleSliderInternal(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
    {
        ...
        if (!temp_input_is_active) {
            ...
            if (input_requested_by_tabbing || clicked || g.NavActivateId == id || g.NavActivatePressedId == id) {
                SetActiveID(id, window);
                SetFocusID(id, window);
                FocusWindow(window);
                g.ActiveIdUsingNavDirMask |= (1 << ImGuiDir_Left) | (1 << ImGuiDir_Right);
                if (temp_input_allowed && (input_requested_by_tabbing || (clicked && g.IO.KeyCtrl) || g.NavActivatePressedId == id))
